...
// 교수 과제 공지 테이블
    ...
    private long assignment_id; // 과제ID(PK)
    ...
    @Column(name = "is_enabled", columnDefinition = "TINYINT(1) NOT NULL DEFAULT 0")
    private short is_enabled; // 제출창구(제출 마감시 비활성화(0), 제출 진행중일시 활성화(1))

    @Column(name = "open_at", columnDefinition = "DATETIME NOT NULL")
    @JsonFormat(pattern = "yyyy-MM-dd")
    private LocalDate open_at; // 제출 시작일

    @Column(name = "due_at", columnDefinition = "DATETIME NOT NULL")
    @JsonFormat(pattern = "yyyy-MM-dd")
    private LocalDate due_at; // 제출 마감일
    ...
    private int max_size_mb; // 파일 최대 용량(mb), 기본값 50
